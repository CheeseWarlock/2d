...
export default class Renderer {
  ...
  drawViewCone() {
    ...
    context.lineTo(this.game.player.x + Math.cos(this.game.viewDirection - this.game.fov) * 1e6, this.game.player.y + Math.sin(this.game.viewDirection - this.game.fov) * 1e6);
    context.lineTo(this.game.player.x + Math.cos(this.game.viewDirection + this.game.fov) * 1e6, this.game.player.y + Math.sin(this.game.viewDirection + this.game.fov) * 1e6);
    ...
    context.beginPath();
    context.moveTo(this.game.player.x, this.game.player.y);
    context.lineTo(this.game.player.x + Math.cos(this.game.viewDirection) * 1e6, this.game.player.y + Math.sin(this.game.viewDirection) * 1e6);
    context.closePath();
    context.stroke();
  }
  ...
  drawGoalCameraFrame() {
    this.drawCamera(this.game.goals[this.game.currentGoalIndex], this.renderingContexts.goalFrame);
  }
  ...
  drawCamera(frame: CameraFrame, target: CanvasRenderingContext2D) {
    ...
    frame.segments.forEach(segment => {
      ...
      target.fillRect(0, segment.start * 1000, 50, (segment.end - segment.start) * 1000)
    })
